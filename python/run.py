        ...
        if check_connection():
            try:
                ...
                if trnx_response and trnx_response.status_code == 200:
                    ...
                    print("Error: Invalid response from transaction API.")
                    return None
            except Exception as e:
                print(f"Error while requesting transactions: {e}")
                return None
        else:
        ...
        if check_connection():
            try:
                ...
                if balance_response and balance_response.status_code == 200:
                    ...
                    print("Error: Invalid response from balance API.")
                    return None
            except Exception as e:
                print(f"Error while requesting balance: {e}")
                return None
        else:
            print(f"-------->>> Trying to establish a connection!!! || Checked: {connection_count} time(s) <<<--------\n")
            time.sleep(10)
    ...
    while looper:
        ...
        for address_index in range(1):
            ...
            for mainnet in check_mainnet:
                ...
                if wallet_trnx_status and "status" in wallet_trnx_status:
                    if wallet_trnx_status["status"] == "1":
                        ...
                        if wallet_trnx_balance and "result" in wallet_trnx_balance and wallet_trnx_balance["result"] != "0":
